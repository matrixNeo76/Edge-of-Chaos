...
```
Edge-of-Chaos/
├── thermodynamic_valence.py       # Python metrology engine (dissipation proxies, k-NN KL-divergence, Psi(t))
...
* **$\sigma_{ex}$ / $\sigma_{hk}$**: In the papers, the excess and housekeeping entropy
  production rates of the Hatano-Sasa NESS decomposition. **In the current code they
  are heuristic proxies, not Hatano-Sasa quantities**: $\sigma_{hk}$ scales as
  $\sigma_{noise}^2/dt$, so it depends on the integration step, and the true housekeeping
  rate of the one-variable digital-twin model is identically zero. Compare $\Psi(t)$
  values only at equal $dt$. A proper estimator (e.g. Sekizawa, Ito & Oizumi, *Phys.
  Rev. X* 14, 041003, 2024) is not implemented yet.
